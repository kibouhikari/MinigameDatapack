# マーカーから半径12.5mに人がいないならタグをつける
    execute in minecraft:overworld as @e[type=marker,tag=rmn.map0,tag=rmn.ffa_random_spawn] at @s unless entity @a[distance=..12.5,tag=rmn.player,gamemode=adventure] run tag @s add rmn.can_spawn
