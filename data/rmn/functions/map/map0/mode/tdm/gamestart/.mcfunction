...
# 初期リス地点を設定
    tag @e[type=marker,tag=rmn.map0,tag=rmn.tdm_random_spawn,limit=1,sort=random,tag=!rmn.blue] add rmn.red
    tag @e[type=marker,tag=rmn.map0,tag=rmn.tdm_random_spawn,limit=1,sort=random,tag=!rmn.red] add rmn.blue

# 参加者をTP
    #tpしたい人にタグをつける
    tag @a[tag=rmn.player] add rmn.need_spawn
    function rmn:map/map0/mode/tdm/gamestart/tp_repeat

# リス地点を再設定
    function rmn:map/map0/mode/tdm/system/spawn/change/red/
    function rmn:map/map0/mode/tdm/system/spawn/change/blue/
