# 死亡処理
    ...
    execute as @r[tag=rmn.death] run function rmn:map/map2/mode/ffa/system/death/
...
# キル数を表示
    execute as @a[tag=rmn.player] run title @s actionbar [{"text":"キル数:"},{"color":"aqua","score":{"objective":"rmn.killcount","name": "@s"}},{"text":"トップのキル数:"},{"color":"aqua","score":{"objective":"rmn.temporary","name": "#most_killcount"}}]

# 試合時間が0なら試合を終了させる
    execute if score #ffa_gametimer rmn.temporary matches ..0 run function rmn:map/map2/mode/ffa/gameend/