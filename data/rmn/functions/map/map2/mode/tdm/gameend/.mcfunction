...
# リス地点をリセット
    tag @e[type=marker,tag=rmn.map2,tag=rmn.tdm_random_spawn] remove rmn.red
    tag @e[type=marker,tag=rmn.map2,tag=rmn.tdm_random_spawn] remove rmn.blue
...
# 試合が始まったことを通知
    ...
    #勝利者を表示
    execute unless data storage rmn: {on_start:1b} run function rmn:map/map2/mode/tdm/gameend/win/
