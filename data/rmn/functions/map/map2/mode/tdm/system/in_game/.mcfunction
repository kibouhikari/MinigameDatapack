# スポーン地点変更
    execute if score #tdm_change_spawn_tick rmn.temporary matches ..0 run function rmn:map/map2/mode/tdm/system/spawn/change/red/
    execute if score #tdm_change_spawn_tick rmn.temporary matches ..0 run function rmn:map/map2/mode/tdm/system/spawn/change/blue/
...
# 死亡処理
    ...
    execute as @r[tag=rmn.death] run function rmn:map/map2/mode/tdm/system/death/
...
# 試合時間が0なら試合を終了させる
    execute if score #tdm_gametimer rmn.temporary matches ..0 run function rmn:map/map2/mode/tdm/gameend/