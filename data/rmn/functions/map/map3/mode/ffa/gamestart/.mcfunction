...
# 参加者をTP
    ...
    function rmn:map/map3/mode/ffa/gamestart/tp_repeat

# ロードアウト振り分け
    execute as @a[tag=rmn.player] run function rmn:map/map3/mode/ffa/system/get_loadout
...
# 試合時間を適用
    ...
    scoreboard players set #ffa_gametimer_tick rmn.temporary 20
...
# 試合が始まったことを通知
    ...
    title @a subtitle [{"text":"- FFA -","color":"gray"}]
