...
# エントリー者を参加者へ変更
    ...
    #参加者のマップidを更新
    scoreboard players set @a[tag=rmn.player] rmn.in_map 3
...
# チーム変更
    ...
    function rmn:map/map3/mode/tdm/system/team_random/

# 初期リス地点を設定
    tp @a[team=rmn.red] @e[type=marker,tag=rmn.map3,tag=rmn.tdm_random_spawn,tag=rmn.first_red_spawn,limit=1,sort=random]
    tp @a[team=rmn.blue] @e[type=marker,tag=rmn.map3,tag=rmn.tdm_random_spawn,tag=rmn.first_blue_spawn,limit=1,sort=random]

# 参加者をTP
    #tpしたい人にタグをつける
    tag @a[tag=rmn.player] add rmn.need_spawn
    function rmn:map/map3/mode/tdm/gamestart/tp_repeat

# ロードアウト振り分け
    execute as @a[tag=rmn.player] run function rmn:map/map3/mode/tdm/system/get_loadout
