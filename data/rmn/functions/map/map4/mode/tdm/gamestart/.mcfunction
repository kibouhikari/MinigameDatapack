...
# エントリー者を参加者へ変更
    ...
    #参加者のマップidを更新
    scoreboard players set @a[tag=rmn.player] rmn.in_map 4
...
# チーム変更
    ...
    function rmn:map/map4/mode/tdm/system/team_random/

# 初期リス地点を設定
    tag @e[type=marker,tag=rmn.map4,tag=rmn.tdm_random_spawn,limit=1,sort=random,tag=!rmn.blue] add rmn.red
    tag @e[type=marker,tag=rmn.map4,tag=rmn.tdm_random_spawn,limit=1,sort=random,tag=!rmn.red] add rmn.blue

# 参加者をTP
    #tpしたい人にタグをつける
    tag @a[tag=rmn.player] add rmn.need_spawn
    function rmn:map/map4/mode/tdm/gamestart/tp_repeat

# リス地点を再設定
    function rmn:map/map4/mode/tdm/system/spawn/change/red/
    function rmn:map/map4/mode/tdm/system/spawn/change/blue/
...
# ロードアウト振り分け
    execute as @a[tag=rmn.player] run function rmn:map/map4/mode/tdm/system/get_loadout
