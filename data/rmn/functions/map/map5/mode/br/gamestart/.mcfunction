...
# エントリー者を参加者へ変更
    ...
    #参加者のマップidを更新
    scoreboard players set @a[tag=rmn.player] rmn.in_map 5
...
# 参加者をTP
    ...
    function rmn:map/map5/mode/br/gamestart/tp_repeat
...
# 試合時間を適用
    scoreboard players set #br_gametimer_1 rmn.temporary 195
    scoreboard players set #br_gametimer_2 rmn.temporary 90
    scoreboard players set #br_gametimer_3 rmn.temporary 45
...
# バトルロワイヤル
    ...
    execute in minecraft:tenkuu run worldborder set 701

    #補給ボックス出現
    function rmn:map/map5/mode/br/system/supply/
