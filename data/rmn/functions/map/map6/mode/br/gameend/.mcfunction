...
# バトルロワイヤルを初期化
    execute in minecraft:tenkuu as @e[type=marker,tag=rmn.map6,tag=rmn.br_center,limit=1,sort=random] at @s run worldborder center ~ ~
...
# 試合が始まったことを通知
    ...
    #勝利者を表示
    execute unless data storage rmn: {on_start:1b} run function rmn:map/map6/mode/br/gameend/win/
